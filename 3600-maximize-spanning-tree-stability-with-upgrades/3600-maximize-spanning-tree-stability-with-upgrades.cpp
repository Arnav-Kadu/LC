...
public:
    ...
    int maxStability(int n, vector<vector<int>>& edges, int k) {
        // no cycle and all n nodes via n-1 edges simple 
        // int n_=n;
        // lets check -1 conditions first 
        // all include edges must result in a spanning tree and no cycle
        // binary search on answer i think thats the only way interesting :)
        // DSU for greedily putting everything and able to combine edges in O(1) and computing it in O(Elogv) using MST
        {
            DSU dsu(n);
            for (auto &e : edges) {
                if (e[3] == 1) {
